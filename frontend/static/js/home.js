...
function news_feed(){
    ...
    fetch(`http://127.0.0.1:8000/api/news_feed/`).then((resp) => resp.json()).then(function(posts){
...
function add_comment(post_id){
    ...
    fetch(`http://127.0.0.1:8000/api/add_comment/${post_id}`, {
        method:'POST', 
        headers:{
...
function add_like(like_id, post_id){
    ...
    if(like_flag){
        fetch(`http://127.0.0.1:8000/api/add_like/${post_id}`, {
            method:'POST', 
            headers:{
                'Content-type':'application/json',
                'X-CSRFToken':csrftoken,
    ...
    {   
        fetch(`http://127.0.0.1:8000/api/delete_like/${like_id}`, {
            method:'DELETE', 
            headers:{
                'Content-type':'application/json',
                'X-CSRFToken':csrftoken,
...
function get_comments(post_id){
    fetch(`http://127.0.0.1:8000/api/get_comments/${post_id}`).then((resp) => resp.json()).then(function(comments){
        ...
        function fill_comments(comment){
            ...
            fetch(`http://127.0.0.1:8000/api/get_profile/${comment.commenter_id}`).then((resp) => resp.json()).then(function(user){
